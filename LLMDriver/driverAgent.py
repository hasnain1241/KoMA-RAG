...
class DriverAgent:
    def __init__(
        ...
        self.llm = create_chat_llm(role="driver", temperature=temperature, max_tokens=2000)

    def few_shot_decision(
        self,
        scenario_description: str = "Not available",
        available_actions: str = "Not available",
        driving_attentions: str = "Not available",
        fewshot_messages: List[str] = None,
        fewshot_answers: List[str] = None,
        previous_plan: str = "Not available",
        master_directive: Optional[str] = None,
    ):
        ...
        if master_directive:
            master_block = f"""
        {delimiter} Master coordination directive (must respect constraints):
        {master_directive}
        """
        ...
        2.If a Master coordination directive is provided, align your local goal with the assigned goal and obey the listed constraints (heuristic soft constraints).
        3.Check whether there is already a previously established plan. 
